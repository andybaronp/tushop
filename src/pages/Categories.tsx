import CategoryCard from '../components/CategoryCard'
import { Category } from '../interfaces/product'

const Categories = () => {
  return (
    <div className='min-h-screen p-6 bg-green-300 mt-14 '>
      <div className='gap-4 md:flex md:justify-between md:flex-wrap'>
        {categorias.map(({ category, name, description }: Category, index) => (
          <CategoryCard
            key={index}
            category={category}
            name={name}
            description={description}
          />
        ))}
      </div>
    </div>
  )
}

export default Categories

const categorias = [
  {
    'category': 'smartphones',
    'name': 'Teléfonos inteligentes',
    'description':
      'Dispositivos móviles de última generación para estar siempre conectado.',
  },
  {
    'category': 'laptops',
    'name': 'Laptops',
    'description':
      'Potentes computadoras portátiles para trabajar o entretenerte donde quieras.',
  },
  {
    'category': 'fragrances',
    'name': 'Fragancias',
    'description': 'Aromas irresistibles que te acompañan a lo largo del día.',
  },
  {
    'category': 'skincare',
    'name': 'Cuidado de la piel',
    'description': 'Productos para mantener tu piel radiante y saludable.',
  },
  {
    'category': 'groceries',
    'name': 'Comestibles',
    'description': 'Artículos básicos y alimentos para el día a día.',
  },
  {
    'category': 'home-decoration',
    'name': 'Decoración del hogar',
    'description': 'Elementos para embellecer y dar personalidad a tu casa.',
  },
  {
    'category': 'furniture',
    'name': 'Muebles',
    'description': 'Piezas que transforman tus espacios en hogares acogedores.',
  },
  {
    'category': 'tops',
    'name': 'Partes superiores',
    'description': 'Prendas de vestir para la parte superior del cuerpo.',
  },
  {
    'category': 'womens-dresses',
    'name': 'Vestidos de mujer',
    'description': 'Ropa elegante y versátil para mujeres.',
  },
  {
    'category': 'womens-shoes',
    'name': 'Zapatos de mujer',
    'description': 'Calzado femenino para todas las ocasiones.',
  },
  {
    'category': 'mens-shirts',
    'name': 'Camisas de hombre',
    'description': 'Prendas masculinas para lucir con estilo.',
  },
  {
    'category': 'mens-shoes',
    'name': 'Zapatos de hombre',
    'description': 'Calzado masculino para cualquier momento.',
  },
  {
    'category': 'mens-watches',
    'name': 'Relojes de hombre',
    'description': 'Accesorios funcionales y sofisticados para hombres.',
  },
  {
    'category': 'womens-watches',
    'name': 'Relojes de mujer',
    'description': 'Piezas elegantes que realzan la belleza femenina.',
  },
  {
    'category': 'womens-bags',
    'name': 'Bolsos de mujer',
    'description': 'Complementos prácticos y estilosos para mujeres.',
  },
  {
    'category': 'womens-jewellery',
    'name': 'Joyería de mujer',
    'description': 'Acentos brillantes que realzan tu estilo personal.',
  },
  {
    'category': 'sunglasses',
    'name': 'Gafas de sol',
    'description': 'Accesorios para proteger tus ojos con estilo.',
  },
  {
    'category': 'automotive',
    'name': 'Automóviles',
    'description': 'Productos y accesorios para el mundo automotriz.',
  },
  {
    'category': 'motorcycle',
    'name': 'Motocicletas',
    'description': 'Emoción y libertad sobre dos ruedas.',
  },
  {
    'category': 'lighting',
    'name': 'Iluminación',
    'description': 'Elementos para dar luz y atmósfera a tus espacios.',
  },
]
